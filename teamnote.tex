...
\author{\texttt{@evenharder}(SANGHEON LEE), \texttt{@SoulTch}(JEONGJIN LEE), \texttt{@djkim0613}(DOJAE KIM)}
...
\texttt{divsum(to, c, k, m)} = $\sum_{i=0}^{\mathrm{to}-1}{\lfloor\dfrac{ki+c}{m}\rfloor}$, \texttt{modsum(to, c, k, m)} = $\sum_{i=0}^{\mathrm{to}-1}{(ki+c) \% m}$
...
// find k s.t. g^k = b mod n
ll bsgs(ll g, ll b, ll n) {
    ll sq = ceil(sqrt(n) + 1e-9);
    vector<pl> prec(sq);
    for(ll i=0;i<sq;i++) {
        prec[i] = {mexp(g, i, n), i};
    }
    sort(prec.begin(), prec.end());
    ll base = mexp(g, n-1-sq, n); // g^(-m), assume n : prime
    ll cur = b;
    for(int i=0;i<sq;i++) {
        auto it = lower_bound(prec.begin(), prec.end(), pl(cur, 0));
        if(it->first == cur) {
            return (i*sq + it->second) % (n-1);
        }
        cur = cur * base % n;
    }
    return -1;
...
\subsection{FFT}
FFT : $
(a_0, a_1, \cdots, a_{n-1}) \mapsto (\sum_{j=0}^{n-1}a_j (\omega^0)^j, \sum_{j=0}^{n-1}a_j (\omega^1)^j, \cdots, \sum_{j=0}^{n-1}a_j (\omega^{n-1})^j)
$
\begin{minted}{cpp}
...
typedef point<ll> P; // use point<double> at caution with fabs, eps
\end{minted}
...
\begin{minted}{cpp}
vector<P> make_hull(vector<P>& v) {
    sort(v.begin(), v.end());
    int n = v.size(), sz = 0;
    vector<P> L, U;
    for(int i=0;i<n;i++) {
        while(sz >= 2 && (L[sz-1]-L[sz-2])/(v[i]-L[sz-1]) <= 0)
            L.pop_back(), sz--;
        L.push_back(v[i]); sz++;
    }
    sz = 0;
    for(int i=n-1;i>=0;i--) {
        while(sz >= 2 && (U[sz-1]-U[sz-2])/(v[i]-U[sz-1]) <= 0)
            U.pop_back(), sz--;
        U.push_back(v[i]); sz++;
    }
...
\begin{minted}{cpp}
ll rotating_calipers(vector<P> v) {
    v = make_hull(v);
    ll mdist = 0;
    int j = 1, n = v.size();
    v.insert(v.end(), v.begin(), v.end());
    for(int i=0;i<n;i++) {
        while((v[i+1]-v[i])/(v[j+1]-v[j]) > 0) j++;
        for(int k=0;k<2;k++)
            mdist = max(mdist, (v[j+k]-v[i]).dist2());
    }
    return mdist;
}
\end{minted}
...
struct line {
    P a, v;
    using intersect = pair<int, P>;
    intersect getIntersect(const line &o) {
        double x = v / o.v;
        if(abs(x) < eps) return intersect(0, P(0, 0));
        return intersect(1, a + v.mult((1 / x) * ((o.a - a) / o.v)));
    }
};
bool g(line &a, line &b, line &c) {
    P p;
    tie(ignore, p) = a.getIntersect(b);
    return (c.v / (p - c.a)) > 0;
}
bool f(int d) {
    vector<line> lines;
    for(int i = 0 ; i < N ; i++) {
        int r = (i + d + 1) % N;
        lines.push_back({v[i], v[r] - v[i]});
    }
    deque<line> dq;
    for(auto l : lines) {
        while(dq.size() >= 2 && g(dq[(int)dq.size() - 1], dq[(int)dq.size() - 2], l)) dq.pop_back();
        while(dq.size() >= 2 && g(dq[0], dq[1], l)) dq.pop_front();
        if(dq.size() >= 2 && g(dq[(int)dq.size() - 1], l, dq[0])) continue;
        dq.push_back(l);
    }
    return dq.size() < 3;
}
...
int main(){
    ...
    for(int i = 1 ; i <= t; i++){
        int ind = 1;
        for(int j = 1 ; j <= N ; j++)
        if( (A[j] - cur) * (A[j] - cur) > (A[ind] - cur)  * (A[ind] - cur)) ind = j;
...
namespace aho_corasick {
    ...
    int go[MAXN+1][MAXC], fail[MAXN+1];
    bool term[MAXN+1];
    ...
    bool query(string &t) {
        ...
        repi(i, t) {
            ...
            p = max(0, step(p, c)); // directly access next including fail
...
\begin{minted}{cpp}
// required macro : rep, repr, repp
// str  : abracadabra
// SA   : 10 7 0 3 5 8 1 4 6 9 2    (0-based)
// LCP  : 1 4 1 1 0 3 0 0 0 2       (lcp[i] : lcp of sa[i], sa[i+1])
...
int dfn[MAXN], low[MAXN], vis[MAXN]; // graph, bcc not mentioned
int piv = 0, cpiv = 1; // reset -> dfs(x, 0) -> color(x, 1)
...
void color(int x, int c){
    ...
    for(int i=0; i<graph[x].size(); i++){
        int w = graph[x][i];
        if(vis[w]) continue;
        if(dfn[x] <= low[w]){ // cut vertex : '<=', bridge : '<'
            bcc[c].push_back(++cpiv);
            bcc[cpiv].push_back(c);
            color(w, cpiv);
        }
        else color(w, c);
    }
...
\begin{minted}{cpp}
int par[16][MAXN], head[MAXN], sz[MAXN], in[MAXN], out[MAXN], lv[MAXN];
vector<int> v[MAXN];
...
void dfs1(int x, int p) {
    ...
    for(int y : v[x]) {
        if(y == p) continue;
        head[y] = y == v[x][0] ? head[x] : y;
        dfs1(y, x);
    }
    out[x] = cnt;
}
int get_query(segtree& seg, int x, int p) {
    int ret = 0;
    while(head[x] != head[p]) {
        // query(in[head[x]], in[x]);
        x = par[0][head[x]];
    }
    // query(in[p], in[x]);
    return ret;
}
// for(int i=1;i<=n;i++) if(!in[i]) head[i] = i, dfs0(i, 0), dfs1(i, 0);
\end{minted}
...
struct Dinic{
    ...
    struct edge{
        T index, cap, rev;
        edge(T index=0, T cap=0, T rev=0) : index(index), cap(cap), rev(rev) {}
    };

    vector<vector<edge>> ADJ;
    vector<int> R, W;

    Dinic(int N=0) : N(N), ADJ(N), R(N), W(N) {}
...
struct MCMF {
    const T INF = 1e9;
    struct edge {
        T there, cap, cost, rev;
        edge(T there=0, T cap=0, T cost=0, T rev=0)
            : there(there), cap(cap), cost(cost), rev(rev) {}
    };
    int N;
    vector<vector<edge>> ADJ;
    vector<T> R, INQ, C, I;

    MCMF(int N=0) : N(N), ADJ(N + 1), R(N + 1), INQ(N + 1), C(N + 1), I(N + 1) {}

    void CE(int i, int j, T cap, T cost) {
    ...
    bool SPFA(int src, int sink) {
        queue<int> Q;    Q.push(src);
        fill(R.begin(), R.end(), -1);       R[src] = 0;
        fill(C.begin(), C.end(), INF);      C[src] = 0;
        ...
        while (Q.size()) {
            ...
            for (int i = 0; i < ADJ[here].size(); i++) {
                auto e = ADJ[here][i];
                if ((C[e.there] == INF || C[e.there] > C[here] + e.cost) &&
                    e.cap > 0) {
...
An array $ P[i][j] $ is \textbf{Monge} array if $\forall a \leq b \leq c \leq d$, $ C[a][c] + C[b][d] \leq C[a][d] + C[b][c]$.
\subsection{Knuth Optimization}
\begin{itemize}
\item Recurrence : $ D[i][j] = \min_{i<k<j}(D[i][k] + D[k][j]) + C[i][j]$
\item Condition : $\forall a \leq b \leq c \leq d$, $C[i][j]$ is Monge array  and $ C[b][c] \leq C[a][d]$ (monotonic)
\item $ A[i][j] = $ (min. $ k $ \textit{s.t.} $ D[i][j] $ is min.). Then $ A[i][j-1] \leq A[i][j] \leq A[i+1][j]$
\item Time Complexity: $ O(N^3) \to O(N^2) $
\end{itemize}
\begin{minted}{cpp}
// opt[i-1][i] = i
for(int d=2;d<=n;d++) {
...
\item Condition: $ C[i][j] $ is Monge array.
\item $ A[t][i] \leq A[t][i+1] $ ($ A[t][i] = $ (min. $ k $ \textit{s.t.} $ D[t][i] $ is min.)) holds
\item Able to Divide and Conquer base on calculating $ D[t][i] $
\item Time Complexity: $ O(TN^2) \to O(TN \lg N) $
\end{itemize}
\begin{minted}{cpp}
// range of index : [l,r]
// range of dp    : [s,e]
...
\begin{itemize}
\item Recurrence : $ dp[i] = \min_{j<i} (dp[j] + a[i]b[j])$
\item Condition: None, but usually $ b[i] $ is monotonic. If not, refer to Section \ref{ss:dynamic-convex-hull}.
\item Time Complexity: $ O(N^2) \to O(N \lg N) $, $ O(N) $ if $ a[i] $ is monotonic by taking valid front.
\end{itemize}
\begin{minted}{cpp}
using ll = long long;
struct line {
...
struct ConvexHull {
    ...
    vector<line> v;
    void insert(line p) {
        while(r - l >= 2 && check_back(v[r-2], v[r-1], p))
            v.pop_back(), r--;
        v.push_back(p);
    }
    pair<ll, ll> query(ll x) { // if x is increasing, else do binary search
        while(r - l >= 2 && check_front(x, v[l], v[l+1]))
            l++;
        return pair<ll, ll>{v[l].calc(x), v[l].i};
    }
};
void calc() { // psuedo-code
    ConvexHull ch;
    ll val = 0, cnt = 0; // current dp and track
    for(int i=0;i<N;i++) {
        ch.insert(line{p[i], q[i]}); // j-dependent values (b[j], dp[j])
        pll res = ch.query(x[i]);
        val = res.first + c[i]; // i-dependent values
        cnt = res.second + 1;
    }
    return pll{val, cnt};
}
...
\begin{itemize}
\item Recurrence: $ dp[i][j] = Min_{k<j} (dp[i-1][k] + C[k+1][j]) $
\item Condition: $ dp[*][n] $ is convex (implied if $ dp[*][n] $ is Monge array)
\end{itemize}
\begin{minted}{cpp}
ll calc(...) {
    // (...)
    k = min(k, (int)v.size());
    ll leff = some_min_value, reff = some_big_value; // check range and overflow
    int lind = v.size(), rind = 1; // range of k, decreases when eff increase
    while(reff - leff >= 2) {
        ll meff = (leff + reff) / 2;
        pll res = calc_with_lambda(v, meff); // {value, k'}
        if(res.second < rind) res.second = rind; // rind (<) lind
        if(res.second > lind) res.second = lind; // lind (>) rind
        if(res.second == k) return res.first - k * meff;
        else if(res.second > k) lind = res.second; leff = meff; // lind (>) rind
        else rind = res.second; reff = meff;
    }
    // when minimizing, ans is max(calc_with_lambda().first - k * eff) for all eff
    // when maximizing, calculate min instead
    return max(calc_with_lambda(v, leff).first - k * leff,
               calc_with_lambda(v, reff).first - k * reff);
}
\end{minted}
...
node head[MAXN];
node* alloc() {
    memset(pool + pcnt, 0, sizeof(node));
    return pool + pcnt++;
}

void init(node *here, int l, int r){
    if(l == r) return;
...
void update(node *par, node *here, int l, int r, int pos, int val){
    here->val += val;
    if(l == r) return;
    ...
    if(pos <= mid) {
        here->l = alloc();
        here->l->val = par->l->val;
        update(par->l, here->l, l, mid, pos, val);
    }
    else {
        here->r = alloc();
        here->r->val = par->r->val;
        update(par->r, here->r, mid + 1, r, pos, val);
    }
}
\end{minted}
...
\subsection{Dynamic Convex Hull}\label{ss:dynamic-convex-hull}
\begin{minted}{cpp}
