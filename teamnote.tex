...
\begin{minted}{cpp}
int par[18][100003], lv[100003] = {-1, };
vector<int> v[100003], g[100003]; // v : original, g : only children
int in[100003], out[100003], siz[100003], head[100003];
void dfs0(int x, int p) {
    par[0][x] = p;
    lv[x] = lv[p] + 1;
    for(int y : v[x])
        if(y != p) dfs0(y, x), g[x].push_back(y);
}
void dfs1(int x) {
    siz[x] = 1;
    for(auto& y : g[x]) {
...
int get_ans(int x, int p) { // lca required
    int ret = 0;
    while(head[x] != head[p]) {
        // range : in[head[x]]], in[x]
        x = par[0][head[x]];
    }
    // range : in[p], in[x];
    return ret;
