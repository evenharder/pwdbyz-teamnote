...
struct MCMF {
    struct edge {
        ...
        edge(int there, int cap, int cost, int rev)
    ...
    MCMF() : N(0) {}
    MCMF(int N) : N(N), ADJ(N + 1), R(N + 1), INQ(N + 1), C(N + 1), I(N + 1) {}
    ...
    pii mcmf(int src, int sink, int ff) {
        pii ret = {0, 0};
        while (ret.first < ff && SPFA(src, sink)) {
            int flow = INF, cost = 0;
